		...
		if( Packet->GetPacketType( ) == W3GS_HEADER_CONSTANT )
		{
			...
			case CGameProtocol :: W3GS_REQJOIN:
				...
				if( m_IncomingJoinPlayer && !m_Banned )
				{
					...
					m_CallableBanCheck = m_Game->m_GHost->m_DB->ThreadedBanCheck( m_Game->GetJoinedRealm( m_IncomingJoinPlayer->GetHostCounter( ) ), m_IncomingJoinPlayer->GetName( ), GetExternalIPString( ), GetSocket( )->GetHostName( ), m_Game->GetOwnerName( ) );
				}
